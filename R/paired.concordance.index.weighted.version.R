    ...
    for (i in seq(from = 1, to = N - 1)) {
      for (j in seq(from = i + 1, to = N)) {
        ...
        if(!missing(weightingFun_obs) & !missing(weightingFun_pred)){
          ...
          w <- abs(log(weightingFun_obs(observations[i] - observations[j]))) * abs(log(weightingFun_obs(predictions[i] - predictions[j])))
        }else if(!missing(weightingFun_obs)){
          w <- abs(log(weightingFun_obs(observations[i] - observations[j])))
        }else{
          w <- 1
        }
