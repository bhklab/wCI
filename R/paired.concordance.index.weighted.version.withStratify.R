...
paired.concordance.index.weighted.version.withStratify <- function(predictions, observations,
  ...
  if(missing(groups)){
    groups <- as.factor(rep("sameType",length(predictions)))
  ...
  if(!CPP){
    ...
    values <- concordanceIndex_modified_helper_weighted_withStratify(x=predictions, y=observations,
                                                        deltaX=delta.pred, deltaY=delta.obs, weightingFun_pred=f_pred, weightingFun_obs=f_obs,
                                                        alpha=alpha, outx=outx, alternative=alternative, logicOp=logic.operator, max_weight, max_weight, permute_weights,groups)
    C <- values$C
    D <- values$D
    CC <- values$CC
    DD <- values$DD
